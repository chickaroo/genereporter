...
def plot_umap(adata: anndata.AnnData = adata, color: str = "celltypist_cell_label_coarse") -> None:
    """
    This function plots a UMAP of the given AnnData object.

    :param adata: The AnnData object to plot. Default is the AnnData object loaded in this module.
...
def clean_data(df: pd.DataFrame, col: str, threshold = 99.75) -> np.ndarray:
    """
...
def classify_exp_level(df, filtered, col) -> Tuple[pd.DataFrame, str]:
    """
...
def make_df(adata: anndata.AnnData, threshold: float=99.75, col: str='log1p(means)', layer: str='log_norm') -> Tuple[pd.DataFrame, str]:
    """
    ...
    filtered = clean_data(df, col=col, threshold=threshold) # can choose upper outlier threshold here
    df_new, summary = classify_exp_level(df =df, filtered=filtered, col=col)
    return df_new, summary


def explain_expr_celltypes(GOI: str, adata: anndata.AnnData=adata, col: str='log1p(means)', layer='log_norm')-> pd.DataFrame:
    """
    ...
    for cell_type in adata.obs['celltypist_cell_label_coarse'].unique():
        ...
        df, = make_df(adata=subset, col=col, layer=layer)
...
def plot_expr_class(GOI: str, ax, adata: anndata.AnnData = adata, cell_type: str = None, col: str = 'log1p(means)') -> Tuple[pd.DataFrame, str]:
    """
...
def plot_expressions(GOI: str, cell_type: str = 'T cell', show_summary: bool = False, adata: anndata.AnnData = adata) -> None:
    """
...
def matrixplot(GOI: str, cell_type: str = None, adata: anndata.AnnData = adata) -> None:
    """
...
def heatmap(GOI: str, layer: str = 'log_norm', adata: anndata.AnnData = adata) -> None:
    """
...
def expression_vs_detection(GOI:str, adata: anndata.AnnData =adata, cell_type: str=None, layer:str='log_norm', col:str='log1p(means)', return_df:bool = False)-> Union[None, pd.DataFrame]:
    """
...
def fit_spline(cell_type: str = None, adata: anndata.AnnData = adata, plot: bool = False) -> Union[None, Tuple[np.ndarray, np.ndarray]]:
    """
...
def calc_distance_point(point: Tuple[float, float], p1: Tuple[float, float], p2: Tuple[float, float]) -> float:
    """
...
def calc_distance(x: pd.Series, y: pd.Series) -> List[float]:
    """
...
def detect_outliers(cell_type: str = None, outlier_threshold: float = 0.15, adata: anndata.AnnData = adata) -> pd.DataFrame:
    """
...
def plot_outliers(GOI: str, cell_type: str = None, outlier_threshold: float=0.15) -> None:
    """
    ...
    detecter = detect_outliers(cell_type=cell_type, outlier_threshold=outlier_threshold)
...
def list_outliers(cell_type: str = None, head: int = 5) -> pd.DataFrame:
    """
